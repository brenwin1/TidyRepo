...
# Introduction
As coffee aficionados, it always pique my curiousity as to what makes coffee better. 

Luckily, we came across this coffee ratings data set from `#TidyTuesday` challenge which we knew we had to analyze the data. Data set consist of scores from coffee cupping(tasting) done by professionals (although one doesn't have to be a professional to do so).  

To do so, we compared coffee ratings across countries. Since the different countries have different harvesting periods, coffee bean types/colour, altitude, variety etc. 
...
It is hard to tell for example Ethiopia is rated the highest across most categories.   
...
Supported by the radar chart above where radarcharts of individual countries are plotted overlapping one another. It is pretty difficult to tell difference in quality between countries since they all overlap very nicely. This implies a similar coffee profile in all the countries.
...
Next we asked ourselves: maybe processing method would make the difference? which led us to the following analysis. 
...
```
To our surprise, all processing method fared almost similarly! As shown on left-side figure, Washed/Wet and Natural/Dry are less erratic, they have almost equal average scores. 

We then break them down into the different categories. They also achieved very similar scores in all categories!


# Conclusion
We found that coffee beans performed rather similarly regardless of different factors in growing coffee beans across countries and even processing method. The best rated coffee beans only scored better by a small margin. In other words, coffee is coffee! 
